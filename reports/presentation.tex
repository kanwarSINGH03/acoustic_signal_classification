...
\begin{frame}{Label Smoothing}
  \scriptsize
  \begin{itemize}
    \item \textbf{Problem:} Hard labels (0 or 1) make the model overconfident and reduce generalization.
    \item \textbf{Idea:} Distribute a small amount of probability mass away from the true class.
    \item In PyTorch (\texttt{CrossEntropyLoss} with \texttt{label\_smoothing=$\varepsilon$}):  
    \[
    ...
    \begin{itemize}
      \scriptsize
      \item $y_i$ = one-hot target (0 or 1),
      \item $K$ = number of classes (here $K=2$),
      \item $\varepsilon$ = smoothing parameter (e.g.\ 0.1).
    \end{itemize}
    \item For binary classification ($K=2$, $\varepsilon = 0.1$):  
    \[
      y = 1 \;\;\Rightarrow\;\; [0.95, 0.05]
      \]
    \[
      y = 0 \;\;\Rightarrow\;\; [0.05, 0.95]
    \]
    \item \textbf{Effect:} Reduces overconfidence, encourages better generalization, and improves calibration.
  \end{itemize}
\end{frame}
...
% --- NEW SLIDE: Threshold analysis plots (after Phase 1 ResNet Performance) ---
\begin{frame}{Phase 1 ResNet — Threshold Analysis (Scores)}
  \centering
  % Top: Scores by index for true = drummy (0)
  \includegraphics[width=0.92\textwidth]{../assets/final_report_assets/p1_scores_by_index_drummy.png}\\[0.35em]
  % Middle: Scores by index for true = tight (1)
  \includegraphics[width=0.92\textwidth]{../assets/final_report_assets/p1_scores_by_index_tight.png}\\[0.35em]
  % Bottom: Distribution of scores by true class
  \includegraphics[width=0.4\textwidth]{../assets/final_report_assets/p1_score_distribution_hist.png}
\end{frame}
...
% --- NEW SLIDE: MFCC + ResNet threshold analysis (scores) ---
\begin{frame}{Phase 2 — MFCC + ResNet: Threshold Analysis (Scores)}
  \centering
  % Top: Scores by index for true = drummy (0)
  \includegraphics[width=0.92\textwidth]{../assets/final_report_assets/p2_mfcc_resnet_scores_by_index_drummy.png}\\[0.35em]
  % Middle: Scores by index for true = tight (1)
  \includegraphics[width=0.92\textwidth]{../assets/final_report_assets/p2_mfcc_resnet_scores_by_index_tight.png}\\[0.35em]
  % Bottom: Distribution of scores by true class
  \includegraphics[width=0.40\textwidth]{../assets/final_report_assets/p2_mfcc_resnet_score_distribution_hist.png}
\end{frame}
